...
def pretty_print(data, indent=4):
    ...
    if type(data) == dict:
        print
        json.dumps(data, indent=indent, sort_keys=True, ensure_ascii=False).encode('utf8')
    else:
        print
        data
...
def article_overview(kind, period):
    ...
    #     if 'media' in article:
    #         for emedia in article['media']:
    #             for emetadata in emedia['media-metadata']:
    #                 if emetadata['format'] == 'Standard Thumbnail':
    #                     urls.append(emetadata['url'])
    # return titles, urls
